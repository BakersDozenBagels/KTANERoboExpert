...
public class Maze : RoboExpertModule
{
    ...
    public override Grammar Grammar => _grammar ??= new(new GrammarBuilder(new Choices("circle", "square", "triangle").ToGrammarBuilder().Then(new Choices(NATO.Take(6).ToArray())).Then(new Choices("1", "2", "3", "4", "5", "6")), 1, 4));
